...
def test_newlines_preserved_multi_line():
    ...
    assert len(result.splitlines()) == len(text.splitlines()), "Line structure must be preserved"
...
def test_newlines_preserved_with_unicode():
    ...
    assert len(result.splitlines()) == len(text.splitlines()), "Line structure preserved"
...
def test_newlines_preserved_with_tabs():
    ...
    text = "Line 1\n\tIndented line\nLine 2\n"
    result = clean_text(text)
    assert "\n" in result and "\t" in result, "Both newlines and tabs should be preserved"
...
def test_newlines_preserved_empty_lines():
    ...
    assert len(result.splitlines()) >= len(text.splitlines()), "Empty line structure preserved"
