# Unicode Text Cleaner (`cleanup-text`) - v1.1.9
...
- `--metrics`                      Include experimental semantic metrics and imply report mode
...
- **--metrics:** Requires NLP extras (`./setup.sh --nlp`) for semantic analysis and automatically switches to report mode
- **--metrics-help:** Print legend without running a report

### Semantic Metrics (preview)

Enabling `--metrics` appends a `metrics` block that captures entropy, ASCII ratio, type/token diversity, heuristic AI-likeness scoring, and more, and automatically switches into report mode. The feature requires the optional NLP extras (`./setup.sh --nlp`) to supply the NLTK resources used for tokenization. Use `--metrics-help` for a quick legend explaining each metric and the ↑/↓ direction hints shown in the report. Pair with `--exit-zero` if you want to surface the data without failing pre-commit.
